...
async def search_products(
    ...
    if not search:
        return [
            {k: v for k, v in prod.items() if k != "embedding"}
            for prod in PRODUCT_EMBEDDINGS[:limit]
        ]
    search_emb = EMBEDDING_MODEL.encode([search])[0]
    # Vectorized cosine similarity
    norms = np.linalg.norm(PRODUCT_EMBED_MATRIX, axis=1) * np.linalg.norm(search_emb)
    scores = np.dot(PRODUCT_EMBED_MATRIX, search_emb) / norms
    top_idx = np.argsort(scores)[::-1][:limit]
    return [
        {k: v for k, v in PRODUCT_EMBEDDINGS[i].items() if k != "embedding"}
        for i in top_idx
    ]
...
async def search_countries(
    ...
    """Returns countries matching search term or by embedding similarity, but does not return embeddings."""
    if not search:
        return [
            {k: v for k, v in country.items() if k != "embedding"}
            for country in COUNTRY_EMBEDDINGS[:limit]
        ]
    search_emb = EMBEDDING_MODEL.encode([search])[0]
    norms = np.linalg.norm(COUNTRY_EMBED_MATRIX, axis=1) * np.linalg.norm(search_emb)
    scores = np.dot(COUNTRY_EMBED_MATRIX, search_emb) / norms
    top_idx = np.argsort(scores)[::-1][:limit]
    return [
        {k: v for k, v in COUNTRY_EMBEDDINGS[i].items() if k != "embedding"}
        for i in top_idx
    ]
